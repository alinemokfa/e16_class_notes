...
Sublime Text has a really cool feature where, if you select a word in text it highlights all occurrences of that word. This is really useful, for example, when you have a function definition and need to see where that function is called.

For some reason, the Atom developers left out this really cool feature. And so, because the Atom community is awesome, someone went and wrote a package that adds the feature.
...
This is the name of a hash-like object, and the key will be a string which represents the key combination we want to use:
