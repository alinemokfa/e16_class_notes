...
- Add a name property with the value `'Shaggy'` to the object that is returned from our constructor
...
To do this we can use the `this` keyword, give our method a name and then assign it's value to an anonymous function. We can then access the method using the dot (`.`) notation, just like we would if we were accessing a property, and invoke the method by adding brackets (`()`) for our argument list.
